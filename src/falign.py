...
_REPEAT_ZONE_HEALTH = 0.35   # мин. доля forced-слов зоны, бьющихся с акустикой (иначе возврат ложный)
_REPEAT_HEALTH_RADIUS = 2    # ± слов вокруг остановки для замера здоровья (узко = точнее разделяет)
    ...
    try:
        ...
        for i in range(n - 1):
            ...
            if fwd and _sim(dec, fwd) >= best[0] - _REPEAT_FWD_MARGIN:
                continue
            # АКУСТИЧЕСКАЯ валидация зоны (rec7: 2 из 3 возвратов были ЛОЖНЫМИ). Возврат детектится
            # на forced-ВРЕМЕНАХ; в зонах мадда/распева они разъезжаются → детектор ловит ложные
            # возвраты на мусорном входе. Проверяем ЗДОРОВЬЕ зоны вокруг слова-остановки i: доля
            # соседних forced-слов, чей greedy-декод в их интервале совпадает со своим скелетом. В
            # здоровой зоне (реальный خضرا 0.67) forced бьётся со звуком → возврату верим; в
            # разъехавшейся (ложные 0.0–0.25) — нет. Опт-аут SYNC_FALIGN_REPEAT_HEALTH=0.
            if os.environ.get("SYNC_FALIGN_REPEAT_HEALTH", "1") != "0":
                lo, hi = max(0, i - _REPEAT_HEALTH_RADIUS), min(n, i + _REPEAT_HEALTH_RADIUS + 1)
                ok = tot = 0
                for k in range(lo, hi):
                    d = _greedy_ctc(emissions, stride_ms, bounds[k][0], bounds[k][1], id2ch)
                    if skel[k] and _sim(d, skel[k]) >= 0.4:
                        ok += 1
                    tot += 1
                if tot and ok / tot < _REPEAT_ZONE_HEALTH:
                    continue
            sim, ra, rb = best
            # перечитка непрерывна: вернувшись к ra, чтец читает подряд до слова-остановки i (откуда
            # ушёл назад), потом продолжает i+1. Декод-матч best даёт лишь начало (rec7: back 11..12
            # при from=13) → без добора перечитка обрывается, а основная дорожка прыгает через
            # недочитанные (12→14, пропущен 13). Дотягиваем rb до i — это НЕ глобальная вставка, а
            # честное свойство возврата (не путать с откаченной замазкой pipeline.fill_forward_jumps).
            rb = max(rb, i)
